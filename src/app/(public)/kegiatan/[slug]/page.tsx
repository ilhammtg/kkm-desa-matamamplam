             ...
             {Array.isArray(post.gallery) && (post.gallery as string[]).length > 0 && (
                <div className="mt-16 mb-12">
                    <div className="flex items-center gap-4 mb-8">
                        ...
                        <h3 className="text-2xl font-bold text-center font-serif text-foreground">Galeri Dokumentasi</h3>
                        <div className="h-px bg-border flex-1" />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {(post.gallery as string[]).map((img, i) => (
                            <div key={i} className="relative aspect-square rounded-2xl overflow-hidden shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 border bg-card">
                                <Image 
                                    src={img} 
                                    alt={`Dokumentasi ${i + 1}`} 
                                    fill 
